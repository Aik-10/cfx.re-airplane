...
$('document').ready(function() {
    ...
    MythicProgBar.Progress = function(data) {
        ...
        $(".progress-container").fadeIn('fast', function() {
            $("#progress-bar").stop().css({"width": 0, "background-color": "rgba(0, 102, 255, 0.75)"}).animate({
              width: '100%'
            }, {
              ...
              complete: function() {
                $(".progress-container").fadeOut('fast', function() {
                    ...
                    $.post('https://progbar/actionFinish', JSON.stringify({
                        })
                    );
                })
              }
    ...
    MythicProgBar.ProgressCancel = function() {
        $("#progress-label").text("KESKEYTETTY");
        ...
        cancelledTimer = setTimeout(function () {
            $(".progress-container").fadeOut('fast', function() {
                $("#progress-bar").css("width", 0);
                $.post('https://progbar/actionCancel', JSON.stringify({
                    })
                );
            });
        }, 1000);
